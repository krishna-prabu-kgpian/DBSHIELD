...
The authorization-bypass experiments showed a clear separation between authentication and authorization. The login flow successfully issued bearer tokens only after valid credentials were presented, and each token was mapped server-side to the corresponding \texttt{username} and \texttt{role}. This means that the later bypass behavior was not caused by fake logins or client-side role spoofing; instead, it came from how the backend handled an otherwise valid authenticated session after token verification.

In the unshielded mode, once a valid token was accepted, selected protected endpoints intentionally skipped role enforcement. As a result, a student token could be reused to access operations that should belong only to instructors or administrators, and a student session could also request records belonging to another user. This confirmed that the vulnerable configuration demonstrates a genuine authorization failure rather than an authentication failure. In the shielded mode, the same requests were rejected because the backend recovered the role from the token mapping and checked it against the allowed role set for each endpoint; student-owned data checks were likewise enforced before returning grade information.

These results are important because they show that secure session handling and secure authorization are related but distinct requirements. The token mapping prevents simple client-side role fabrication, while the per-endpoint role and ownership checks determine whether the authenticated user is actually permitted to proceed. Together, these observations make the project's main lesson concrete: a valid session should prove identity, but it must never by itself imply universal access.
