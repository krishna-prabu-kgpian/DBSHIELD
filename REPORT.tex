...
The first attack category focuses on SQL injection. The purpose of this part is to evaluate whether user-supplied input can alter backend query behavior in ways that compromise authentication, reveal data, or affect database integrity. In the unshielded system, controlled adversarial inputs are supplied to input surfaces such as login-related or database-facing request parameters. The study considers representative categories such as authentication bypass attempts, boolean inference, disclosure-oriented query manipulation, and unauthorized modification attempts.

The same broad categories of tests are then repeated against the shielded system. The expected outcome is that secure handling of database queries prevents the input from changing query semantics, reduces information leakage, and preserves the integrity of stored records. The comparison is therefore based not only on whether the attack succeeds, but also on how safely the application behaves in the face of malformed or adversarial input.
...
This section is currently a placeholder. In the next stage, it can summarize the observed behavior of the unshielded and shielded systems under the three attack categories. The final comparison can include attack success, service degradation, unauthorized access, information exposure, and the effect of the applied safeguards.
