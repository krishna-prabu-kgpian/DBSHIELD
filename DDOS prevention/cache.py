...
class QuerySupersetDetector:
    ...
    Query B is a superset of Query A if:
    - Same table
    - Same or subset of columns
    - B's WHERE conditions include all of A's conditions + more
    """

    def parse_query(self, sql: str) -> Optional[ParsedQuery]:
        """Parse SQL into structured form."""
        try:
    ...
    def _extract_table_name(self, stmt) -> Optional[str]:
        """Extract the main table name from the query."""
        from_seen = False
        for token in stmt.tokens:
            if from_seen:
                if token.ttype is None:  # Identifier
                    # Handle "table" or "schema.table"
                    name = str(token).split()[0]  # Get first word (ignore aliases)
    ...
    def _parse_condition(self, condition_str: str) -> Optional[WhereCondition]:
        ...
        # Match patterns like: column >= 123, column = 'value', column IN (1,2,3)
        patterns = [
            (r'(\w+)\s*(>=|<=|!=|<>|=|>|<)\s*(\d+)', 'numeric'),
            (r"(\w+)\s*(>=|<=|!=|<>|=|>|<)\s*'([^']*)'", 'string'),
            (r'(\w+)\s*(>=|<=|!=|<>|=|>|<)\s*"([^"]*)"', 'string'),
    ...
    def is_superset_of(
        ...
        query_b: ParsedQuery,
        query_a: ParsedQuery
    ) -> Tuple[bool, List[WhereCondition]]:
        """
        Check if query_b is a superset of query_a.

        Returns:
            (is_superset, additional_conditions_to_apply)
        """
        # Must be same table
        if query_b.table_name != query_a.table_name:
            return False, []

        # B's columns must be subset of A's or A is SELECT *
        if query_a.select_columns != ["*"]:
            if query_b.select_columns != ["*"]:
                if not set(query_b.select_columns).issubset(set(query_a.select_columns)):
                    return False, []

        # B must contain ALL of A's conditions
        ...
        # Find additional conditions in B
...
class InMemoryFilter:
    """
    Applies additional WHERE conditions to cached result sets in memory.
    """
    ...
    def filter_results(
        ...
        """
        Filter a cached result set by additional conditions.
        """
        if not conditions:
            return result_set
        ...
        for row in result_set:
            if self._row_matches_all(row, conditions):
                filtered.append(row)

        return filtered

    def _row_matches_all(self, row: Dict[str, Any], conditions: List[WhereCondition]) -> bool:
        ...
        for cond in conditions:
            ...
            if not self._evaluate_condition(column_value, cond.operator, cond.value):
                return False
    ...
    def _evaluate_condition(self, actual: Any, operator: str, expected: Any) -> bool:
        ...
        try:
            if operator == "=":
                ...
                return bool(re.match(f"^{pattern}$", str(actual), re.IGNORECASE))
            else:
                return False
...
class IntermediateResultCache:
    ...
    {
        "TABLE_NAME": {
            "condition_key": IntermediateCacheEntry,
            ...
        }
    }
    """
    ...
    def __init__(
        self,
        max_per_table: int = INTERMEDIATE_CACHE_MAX_PER_TABLE,
        ttl_seconds: int = INTERMEDIATE_CACHE_TTL_SECONDS
    ):
    ...
    async def find_reusable_cache(
        self,
        incoming_query: ParsedQuery
    ) -> Optional[Tuple[List[Dict], List[WhereCondition]]]:
        """
        Find a cached result that can be filtered to satisfy incoming query.

        Returns:
            (cached_result, conditions_to_apply) or None
        """
        async with self._lock:
            ...
            for cache_key, entry in list(self._cache[table].items()):
                ...
                # Check if incoming is superset of cached
                is_superset, additional_conds = self.detector.is_superset_of(
                    incoming_query,
                    entry.parsed_query
                )

                if is_superset:
                    # Score by: fewer additional conditions = better match
            ...
            # Pick best candidate (fewest additional conditions)
    ...
    async def set(self, parsed_query: ParsedQuery, result: List[Dict[str, Any]]):
        """Store result in intermediate cache."""
        async with self._lock:
            ...
            if len(self._cache[table]) >= self.max_per_table:
                # Remove oldest entry
                oldest_key = min(
                    self._cache[table].keys(),
                    key=lambda k: self._cache[table][k].created_at
                )
                del self._cache[table][oldest_key]

            self._cache[table][cache_key] = IntermediateCacheEntry(
    ...
    async def cleanup_expired(self):
        ...
        async with self._lock:
            ...
            for table in list(self._cache.keys()):
                expired = [
                    k for k, v in self._cache[table].items()
                    if current_time - v.created_at > self.ttl_seconds
                ]
                for key in expired:
...
def compute_full_cache_key(sql_query: str) -> str:
    """
    Creates cache key from AST structure + actual parameter values.

    Example:
    - Query: "SELECT * FROM grades WHERE student_id = 123"
    - Produces: hash of normalized structure + parameters
    """
    try:
        ...
        params_str = "|".join(param_values)  # Keep order for exact match
...
def normalize_and_hash(sql_query: str) -> str:
    """
    Original AST normalization for rate limiting.
    Returns hash of structure only (no parameters).
    """
    try:
        parsed = sqlparse.parse(sql_query)
        if not parsed:
