...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\linewidth]{images/ddos_true.png}
...
This project demonstrated that common database-backed application vulnerabilities can be reproduced clearly in a realistic student ERP setting, and that relatively standard defensive measures substantially improve security without changing the core functionality of the system. By comparing an intentionally vulnerable unshielded version with a hardened shielded version of the same application, we were able to isolate the effect of protection mechanisms across three major attack classes: SQL injection, denial of service, and authorization bypass.

The results show a consistent pattern. In the unshielded system, unsafe SQL construction allowed authentication bypass and unauthorized data access, unsafe request handling made the login workflow vulnerable to service degradation under repeated attack traffic, and missing server-side role and ownership checks enabled authenticated users to access actions and records beyond their permissions. In the shielded system, parameterized queries and restricted admin-query handling prevented the tested SQL injection attacks, middleware and bounded processing preserved responsiveness during the login-flood demonstration, and server-side authorization checks successfully blocked cross-role and cross-user access attempts.

Overall, DBSHIELD shows that database security is not a single control but a combination of safe query handling, controlled resource usage, and strict server-side authorization. The side-by-side design of the project makes this especially clear: when these controls are absent, the same application becomes exploitable in multiple ways, but when they are applied carefully, the system remains functional while resisting the demonstrated attacks. This reinforces the broader lesson that security must be treated as a core design requirement in modern database-driven applications, rather than as an optional add-on.
