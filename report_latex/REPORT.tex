...
% PACKAGES
% =========================================================
\usepackage{cmap} % Improve copy/paste fidelity from generated PDF
...
\usepackage{caption}
\usepackage{setspace}
\usepackage{upquote} % Preserve straight ASCII quotes in verbatim/code blocks
\usepackage{xcolor}
...
\begin{center}
{\TitleFont\bfseries
DBSHIELD: DataBase Systems Handling for Injection, Exploitation, Leakage, and DDoS
\par}
...
We focus on three representative classes of attack. The first is \textit{SQL injection}, which targets the interaction between user input and backend query execution. When queries are not handled safely, SQL injection may allow login bypass, unauthorized reads, and unintended modifications to database contents. The second is \textit{distributed denial of service}, where attackers repeatedly send requests to the app, causing genuine users to be stuck in the queue. The third is \textit{authorization bypass}, where a user attempts to access resources or operations outside the limits of their intended role, particularly in systems that trust client-side verification such as cookies over server-side verification.

We also present our github repository for reproduction - \href{https://github.com/krishna-prabu-kgpian/DBSHIELD}{DBSHIELD}
...
In the \textit{Unshielded Implementation}, the raw SQL is directly fed, without any additional checking, to the database to fetch data. This can cause issues such as SQL Injection, DDoS and Authorization Bypass, which are discussed later. The result similarly is directly fed to the user, without verification.

In the \textit{Shielded Implementation}, the input from the frontend is processed, checked for potential SQL Injection attacks, checked for DDoS attacks, and is only then processed. This avoids issues that used to arise in the unshielded implementation.

\subsection{Study Method}

For each of the 3 attack types, we evaluate the unshielded model and the shielded model.

\begin{itemize}
    \item \textbf{SQL Injection:} We test on various SQL injections, and obtain the results for both unshielded and shielded implementations. We verify that shielding did produce significant benefits in mitigating SQL Injection Attacks
    \item \textbf{Distributed Denial of Service:} We test both unshielded and shielded against an attacker spamming the login endpoint to deny service to clients. We verify that in the case of shielding the client did not suffer interruptions to service.
    \item \textbf{Authorization Bypass:} We tested on various attacks to the endpoint for accessing unauthorized information. We verified in the case of shielding such attacks were prevented, but in the case of unshielded the attacker got access to sensitive information. 
\end{itemize}
% =========================================================
% SYSTEM ARCHITECTURE
% =========================================================
...
The frontend is built using React.js, a JavaScript library. It manages state-management, routing, ensuring user data persists across sessions. 

The backend was implemented using FastAPI. It handles communication with frontend and interfacing with database. It contains the parameters for the study - the \texttt{ENABLE\_SQLI\_PROTECTION} toggle, \texttt{ENABLE\_DDOS\_PROTECTION} toggle and \texttt{ENABLE\_AUTH\_BYPASS\_PROTECTION} toggle. Each of the attacks are covered in detail below, discussing how they are implemented and the results obtained. 

The database layer uses SQLite and PostgreSQL. The core schema consists of a Users table, Roles table, Courses table, Enrollments table, Grades table. This completely simulates the Student ERP system. A detailed ER diagram is given below.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.88\linewidth]{images/ER_diagram.png}
...
For controlled testing, the backend variable \texttt{ENABLE\_SQLI\_PROTECTION} is switched between \texttt{False} and \texttt{True} while keeping the same API routes active. A representative login-bypass attempt can be issued against \texttt{/api/login} using a SQLite-compatible tautology payload such as \texttt{' OR '1'='1}. The vulnerable mode is expected to authenticate the attacker or otherwise alter the intended query logic, whereas the shielded mode is expected to reject the request or treat the supplied string as ordinary input. The same comparison is then repeated across course-search, grade-view, enrollment, and admin-query endpoints using the documented payload set from the project tests.
...
When \texttt{ENABLE\_SQLI\_PROTECTION = False}, the expected outcome is a successful login or a query-manipulated response showing that user input has changed SQL semantics. When \texttt{ENABLE\_SQLI\_PROTECTION = True}, the same request is expected to fail safely, either because the payload is detected as malicious or because parameterized handling preserves the intended query structure.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.88\linewidth]{images/SQLi_False.png}
    \caption{Successful SQL injection against the unshielded implementation, showing that the crafted login payload is accepted and query logic is altered.}
    \label{fig:sqli_unshielded}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.88\linewidth]{images/SQLi_True.png}
    \caption{Shielded SQL injection handling with \texttt{ENABLE\_SQLI\_PROTECTION = True}, where the malicious payload is rejected or treated as ordinary input without altering query structure.}
...
Next, we investigate an application-level distributed denial-of-service scenario centered on repeated login traffic. The \texttt{/api/login} endpoint is intentionally costly in the vulnerable configuration because it uses the \texttt{handle\_student\_login()} path, which introduces an artificial delay before issuing the database query. When \texttt{ENABLE\_DDOS\_PROTECTION = False}, every request, including attacker-generated requests with invalid credentials, is allowed to execute this delayed path. Under sustained flooding, these attacking login attempts consume backend worker capacity and cause legitimate users to experience heavy slowdown.

In the shielded configuration, the same login endpoint and user-facing workflow are preserved, but a DDoS protection layer is enabled around the application. In the case of repeated attacks from a fixed IP Address, a rate limiting mechanism is used, that blocks any further than 10 requests a second, and 100 requests a minute. This prevents any Denial of Service attack from a single IP Address. In the case of a Distributed attack, the attack script continuously sends randomized login attempts and includes spoofed \texttt{X-Forwarded-For} headers to imitate varying source IPs. In the current local deployment, the middleware treats such client-supplied forwarded headers as untrusted and rejects those requests before the expensive login logic is reached. Legitimate browser traffic does not include this header and therefore continues through the normal path. In addition, the shielded login flow places a bounded semaphore around expensive authentication work, preventing bursts from creating unbounded concurrent load. The comparison is intended to show that, without protection, attacker traffic is able to consume the costly login path, whereas with protection enabled, spoofed requests are blocked early enough to preserve normal responsiveness for genuine users.
\paragraph{Demonstration setup.}
The demonstration can be reproduced locally by launching the backend, frontend, and attack script from their respective directories in different terminals:
\begin{quote}
\small\ttfamily
Terminal 1:\\
cd backend \\
...
A second case demonstrates a direct single-peer flood without any \texttt{X-Forwarded-For} header:
\begin{quote}
\small\ttfamily
Terminal 3:\\
cd ddos\_attack \\
python3 single\_peer\_attack.py
\end{quote}
In this case, the requests are treated as ordinary direct traffic from one real peer IP. When \texttt{ENABLE\_DDOS\_PROTECTION = True}, the rate limiter is applied to that real source address. Since both the attacker script and the browser are running from localhost in the demo setup, they share the same \texttt{127.0.0.1} rate-limit bucket. Once the single-peer flood exceeds the configured threshold, the legitimate browser user may also be temporarily banned along with the attacker.
...
\centering
\small
\caption{Observed qualitative behavior of the DDoS demonstration.}
\label{tab:ddos_results}
\begin{tabular}{%
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\linewidth]{images/ddos_false.png}
    \caption{Unshielded login endpoint during the spoofed request flood. With \texttt{ENABLE\_DDOS\_PROTECTION = False}, attacker requests still reach the delayed authentication path and are rejected only after backend work is spent, producing many HTTP 401 responses and noticeably degrading responsiveness for legitimate users.}
    \label{fig:ddos_unshielded}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.78\linewidth, height=0.32\textheight, keepaspectratio]{images/ddos_true.png}
    \caption{Shielded login endpoint under the same attack. With \texttt{ENABLE\_DDOS\_PROTECTION = True}, spoofed requests carrying untrusted \texttt{X-Forwarded-For} headers are blocked early in middleware, producing HTTP 403 responses, while legitimate browser requests continue to succeed.}
    \label{fig:ddos_shielded}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.82\linewidth]{images/dos_single_peer_true.png}
    \caption{Shielded single-peer flood without \texttt{X-Forwarded-For}. Because both the attacker script and the browser user originate from localhost in this demo, the direct-IP rate limiter treats them as the same peer and temporarily bans that shared source after the threshold is exceeded.}
    \label{fig:dos_single_peer_shielded}
\end{figure}
...
This comparison highlights the core takeaway of the authorization-bypass study: authentication alone is not sufficient. Even when a token correctly represents a real logged-in user, the backend must still verify that the role attached to that token is permitted to perform the requested action and, where necessary, that the requested data belongs to that user.

\paragraph{Testing procedure.}
For testing, the variable \texttt{ENABLE\_AUTH\_BYPASS\_PROTECTION} in the backend is switched between \texttt{False} (unshielded authorization behavior) and \texttt{True} (shielded authorization behavior). A typical test first logs in as a student to obtain a bearer token and then reuses that token against an endpoint that should require a stronger role. For example:
...
\section{Conclusion}
This project showed that common database-backed application vulnerabilities can be reproduced clearly in a realistic student ERP system, and that standard defensive measures can greatly improve security without changing core functionality. By comparing an intentionally vulnerable unshielded version with a hardened shielded version of the same system, we isolated the impact of protections across three major attack classes: SQL injection, denial of service, and authorization bypass. In the unshielded system, unsafe SQL construction enabled authentication bypass and unauthorized data access, unsafe request handling made the login workflow vulnerable to slowdown under repeated attack traffic, and missing server-side authorization checks allowed authenticated users to exceed their intended permissions.
In contrast, the shielded system prevented the tested SQL injection attacks through parameterized queries and restricted admin-query handling, preserved responsiveness during the login-flood demonstration through middleware and bounded processing, and blocked cross-role and cross-user access through strict server-side authorization checks. Overall, DBSHIELD shows that database security depends on multiple layers of defense working together, including safe query handling, controlled resource usage, and strong authorization enforcemen thus reinforcing security must be treated as fundamental design requirement
\end{document}
